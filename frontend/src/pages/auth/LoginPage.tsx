...
export function LoginPage() {
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      setAuth(data.data);
      notify({ title: 'Selamat datang kembali', description: 'Login berhasil' });
      navigate('/dashboard');
    } catch (error) {
      notify({ title: 'Login gagal', description: 'Periksa kembali kredensial', status: 'error' });
    }
  };
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-primary-50 to-secondary-50">
      ...
      >
        ...
        <button
          type="submit"
          className="w-full rounded-md bg-primary-500 px-4 py-2 text-white shadow-soft hover:bg-primary-600"
        >
          Masuk
        </button>
        <div className="text-center text-sm text-[var(--color-muted)]">
          Belum punya akun? <Link to="/register" className="text-primary-600">Daftar</Link>
        </div>
      </form>
    </div>
  );
